import argparse
from django.core.management import BaseCommand

from django.contrib.auth.models import User, Permission
from django.test import TestCase
from django.test import Client
from django.urls import reverse
from django.utils.timezone import now

from matricula.models import Category, Course, Period, Group, Enroll, Student
from membership_core.models import SystemCurrency, Country


class PoblateDatabase:
    user_count =10

    def get_enroll(self, group):
        user = User.objects.create_user(username='user%d'%self.user_count,
                                 email='jacob%d@upo.com'%self.user_count,
                                 password='password')
        self.user_count += 1
        country, created = Country.objects.get_or_create(name='Costa Rica', flag='cr', code='CRC')
        student = Student.objects.create(
            user =  user,
            organization = 'org',
            country = country,
            city = 'CR',
            phone_number = '88888888',
            expired_at = now()
        )

        enroll = Enroll.objects.create(
            enroll_finished = True,
            enroll_activate = True,
            group = group,
            student = student,
            course_status = "approved")
        return enroll

    def generate_scenarius(self):
        cat1 = Category.objects.create(name='category1', description='cat1')
        cat2 = Category.objects.create(name='category2', description='cat2')
        cat3 = Category.objects.create(name='category3', description='cat3')
        c1 = Course.objects.create(category=cat1, name='course1', content='course1')
        c2 = Course.objects.create(category=cat2, name='course2', content='course2')
        c3 = Course.objects.create(category=cat3, name='course3', content='course3')
        period = Period.objects.create(name='per1', start_date=now(), finish_date=now())

        c1g1 = Group.objects.create(
                period = period,
                course = c1,
                name = 'g1',
                schedule = 'Nothing',
                pre_enroll_start = now(),
                pre_enroll_finish = now(),
                enroll_start = now(),
                enroll_finish = now(),
                cost = 0,
                maximum = 200,
                is_open = True,
                flow = 0)

        c1g2 = Group.objects.create(
                period = period,
                course = c1,
                name = 'g2',
                schedule = 'Nothing',
                pre_enroll_start = now(),
                pre_enroll_finish = now(),
                enroll_start = now(),
                enroll_finish = now(),
                cost = 0,
                maximum = 200,
                is_open = True,
                flow = 0)

        c2g1 = Group.objects.create(
                period = period,
                course = c2,
                name = 'g1',
                schedule = 'Nothing',
                pre_enroll_start = now(),
                pre_enroll_finish = now(),
                enroll_start = now(),
                enroll_finish = now(),
                cost = 0,
                maximum = 200,
                is_open = True,
                flow = 0)

        c2g3 = Group.objects.create(
            period=period,
            course=c2,
            name='g3',
            schedule='Nothing',
            pre_enroll_start=now(),
            pre_enroll_finish=now(),
            enroll_start=now(),
            enroll_finish=now(),
            cost=0,
            maximum=200,
            is_open=True,
            flow=0)

        c3g1 = Group.objects.create(
            period=period,
            course=c3,
            name='g1',
            schedule='Nothing',
            pre_enroll_start=now(),
            pre_enroll_finish=now(),
            enroll_start=now(),
            enroll_finish=now(),
            cost=0,
            maximum=200,
            is_open=True,
            flow=0)

        c3g3 = Group.objects.create(
            period=period,
            course=c3,
            name='g3',
            schedule='Nothing',
            pre_enroll_start=now(),
            pre_enroll_finish=now(),
            enroll_start=now(),
            enroll_finish=now(),
            cost=0,
            maximum=200,
            is_open=True,
            flow=0)

        groups = [c1g1, c1g2, c2g1, c2g3, c3g1, c3g3]
        enroll0 = self.get_enroll(groups[0])
        enroll0.go_to_one_class = True
        enroll0.course_status = 'reproved'

        enroll1 = self.get_enroll(groups[1])
        enroll1.go_to_one_class = True
        enroll1.course_status = 'approved'

        enroll2 = self.get_enroll(groups[2])
        enroll2.go_to_one_class = True
        enroll2.course_status = 'approved'

        enroll3 = self.get_enroll(groups[3])
        enroll3.go_to_one_class = True
        enroll3.course_status = 'approved'

        enroll4 = self.get_enroll(groups[4])
        enroll4.go_to_one_class = True
        enroll4.course_status = 'reproved'

        enroll5 = self.get_enroll(groups[5])
        enroll5.go_to_one_class = True
        enroll5.course_status = 'reproved'

        enroll0.save()
        enroll1.save()
        enroll2.save()
        enroll3.save()
        enroll4.save()
        enroll5.save()



class Command(BaseCommand):
    help = 'load database with dummy objects'

    def handle(self, *args, **options):
        db1 = PoblateDatabase()
        db1.generate_scenarius()
