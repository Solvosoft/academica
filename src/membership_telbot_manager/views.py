...
bot.set_webhook(url="https://ea4539b9.ngrok.io/telbot/")
...
def state(message):
    ...
    if telgroup:
        invoices = Invoice.objects.filter(
            ...
            status='pending')
        if invoices:
            msgg = render_to_string('invoices.txt', {'invoices':invoices,'option': "pendientes",'title':invoices.first().membership.name})
        else:
            invoices = Invoice.objects.filter(
                Q(membership__state='active') | Q(membership__state='inactive'),
                membership__organization__telgroup__pk=telgroup.pk,
                status='paid').order_by("payment_date").last()
            msgg = render_to_string('invoices.txt', {'invoice':invoices,'option': "pagas",'title':invoices.membership.name})
...
def memberships_list(message):
    ...
    if telgroup:
        ...
        if memberships:
            msgg = render_to_string('memberships.txt', {'memberships':memberships,'option': "membresias",'title':telgroup.organization.name})
        else:
            new_text = "No cuenta con membresias.\n"
            msgg = render_to_string('memberships.txt', {'memberships':new_text,'option': "none",'title':telgroup.organization.name})
...
def memberships_services_list(message):
    ...
    if telgroup:
        ...
        if memberships:
            msgg = render_to_string('memberships.txt', {'memberships':memberships,'option': "services",'title':telgroup.organization.name})
        else:
            new_text = "No cuenta con membresias.\n"
            msgg = render_to_string('memberships.txt', {'memberships':new_text,'option': "none",'title':telgroup.organization.name})
...
def left_chat_member(message):
    ...
    telgroup = TelGroup.objects.filter(chat_id=message.chat.id).first()
    if telgroup:
        teluser = TelegramUser.objects.filter(telegram_id=message.from_user.id,groups__chat_id=telgroup.chat_id).first()
