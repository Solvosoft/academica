...
from membership_manager.models import Membership, ActivityReport, Invoice
...
def save_attention(sender, instance, **kwargs):
    ...
    if kwargs['update_fields'] is not None and\
            'duration' in kwargs['update_fields']:
        instance.__class__.objects.filter(
            pk=instance.pk).update(manual_edited=True)
    ...
    if not instance.manual_edited or instance.duration == 0:
        ...
        instance.__class__.objects.filter(
            pk=instance.pk).update(duration=int(total))
