...
class CreditCardField(forms.IntegerField):
    def clean(self, value):
        ...
        if value and (len(value) < 13 or len(value) > 16):
            raise forms.ValidationError(_("Please enter in a valid " + \
                "credit card number."))
...
class SalePaymentForm(forms.Form):
    ...
    bill = forms.CharField(widget=forms.HiddenInput, required=True)
    ...
    def clean(self):
        ...
        if not self.errors:
            ...
            billid = self.cleaned_data["bill"]
 
            sale = Sale()
            bill = Bill.objects.get(pk=billid)
            # let's charge $10.00 for this particular item
            success, instance = sale.charge(bill.amount, bill, number, exp_month,
                                                exp_year, cvc)
